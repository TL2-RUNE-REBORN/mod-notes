...
- 期望量级:1 颗聚灵 ≈ 9 玉髓(约 400 只精英/BOSS),1 颗凝华 ≈ 81 玉髓(约 3,600 只)。11% 凝华单次仅 **1.9%**,凑满 9 枚做太虚属毕生目标级投入,途中开出的低档凝华照常可用。
...
两条铸造线共用的根料全部由怪物掉落、可堆叠,掉率**按怪物强度分层**——越硬的怪,越是材料的来源。下表为**每只怪的实际概率**(已按最新一轮扩表校准)。

| 材料 | 归属 | 普通 | 小精英 | 精英 | BOSS | 群魔堕落 ★ |
|---|---|:-:|:-:|:-:|:-:|:-:|
| <img class="gem-px" src="/img/gems/jade.png" alt=""> 青莲玉髓 | 冷却线 · 根料 | 0.008% | **2.23%** | **2.23%** | **2.23%** | **4.46%** |
| <img class="gem-px" src="/img/gems/jiehui.png" alt=""> 青莲·劫灰 | 突破线 · 通用根料 | 0.004% | 0.56% | 1.12% | **3.16%** | **6.32%** |
| 五系符(指定某一种) | 突破线 · 引子 | — | 0.037% | 0.074% | **0.21%** | **0.42%** |
| <img class="gem-px" src="/img/gems/xiangquan.png" alt=""> 御兽嘲讽项圈 | 御兽嘲讽 | — | — | — | **1%** | **2%** |
| <img class="gem-px" src="/img/gems/modian.png" alt=""> 魔典:御兽·嘲讽令 | 御兽嘲讽 | — | — | — | **1%** | **2%** |

★ **群魔堕落列**指该组件的【史诗级】与【传说级】BOSS —— 那批怪本就"为高手准备",材料掉率给**双倍**。其余怪(堕落英雄 / 勇士 / 战神)按左侧普通 / 精英 / BOSS 档。

- **玉髓是两段形态**:普通杂兵几乎不掉(0.008%),真正的产出在**小精英及以上,三档同为 2.23%**。想攒冷却线,刷精英比刷杂兵海效率高两个数量级。
- **劫灰**全等级掉、越强越多;每次命中固定 1 枚(BOSS 是 3.16% 命中,不是必掉)。
...
> **两线难度对齐**:单颗凝华 ≈ 81 玉髓 ≈ **3,600 只精英/BOSS**(打【史诗·传说级】约 1,800 只);单颗突破石 ≈ 9 劫灰 + 1 指定符,**瓶颈在符** ≈ **1,300 只精英 / 470 只 BOSS / 240 只【史诗·传说级】**——中段互为对标;顶配(太虚 / 单属性顶满 4–8 颗)同为毕生目标。
