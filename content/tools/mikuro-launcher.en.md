...
- **Point-spending / socketing crash safeguard**: crashes when spending points or socketing gems on passive-heavy classes (e.g. 狂热杀神, 独角兽) are now **sealed off at the engine level** — if an effect is destroyed mid stat-recompute, that entry counts as 0 and the recompute carries on instead of reading freed memory. **Independent of any MOD**: every class and MOD combination is covered; on healthy data it never fires and no value is affected.
...
- **Overlay conflict warning.** The loot filter and DXVK both hook D3D9; overlay / recording tools fighting for the same hook can stop the game from starting, so the launcher warns you to close them.
