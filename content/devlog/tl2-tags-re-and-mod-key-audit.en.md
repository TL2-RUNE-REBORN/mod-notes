...
**Angle one: all 56 rghash callers, one by one.** Decompile every one of `sub_4C9FE0`'s 56 call sites and pull the string constants passed in. They classify, without exception, as **DAT/LAYOUT field-key parsers plus the `TAGS.DAT` loader**, with no "magic tag" anywhere:
